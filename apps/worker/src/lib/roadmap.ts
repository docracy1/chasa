...
export interface RoadmapFeature {
   ...
   *  omitted entirely from the admin listing, which has no single voterId to check against. */
  myVote: "yes" | "no" | null;
}
...
export async function createRoadmapFeature(
  ...
  await env.CHASA_DB.prepare(
    ...
    .bind(id, title.trim().slice(0, 200), description.trim().slice(0, 1000), new Date().toISOString())
    .run();
  return { ok: true, id };
}
...
export async function deleteRoadmapFeature(env: Env, id: string): Promise<void> {
  await env.CHASA_DB.prepare(`DELETE FROM roadmap_votes WHERE feature_id = ?`).bind(id).run();
  await env.CHASA_DB.prepare(`DELETE FROM roadmap_features WHERE id = ?`).bind(id).run();
}
...
export async function castRoadmapVote(
  ...
  const feature = await env.CHASA_DB.prepare(`SELECT id FROM roadmap_features WHERE id = ?`)
    .bind(featureId)
    .first();
