...
connector.get("/keys", requirePaidAccount, async (c) => {
  ...
  const keys = await listApiKeys(c.env, acc.id);
  return c.json({ keys });
});
...
connector.post("/keys", requirePaidAccount, async (c) => {
  ...
  const existing = await listApiKeys(c.env, acc.id);
  if (existing.length >= 5) {
    return c.json({ error: "Maximum 5 API keys per account." }, 400);
  }
  const created = await createApiKey(c.env, acc.id, name);
  // token shown once
...
connector.delete("/keys/:id", requirePaidAccount, async (c) => {
  const acc = c.get("account")!;
  const ok = await revokeApiKey(c.env, acc.id, c.req.param("id"));
